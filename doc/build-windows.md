...
The options known to work for building Cascoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Cascoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Cascoin Core applications.
...
Note that for WSL the Cascoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/cascoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/cascoin-project/cascoin.git
    sudo chmod -R a+rw cascoin
...
way. This will install to `c:\workspace\cascoin`, for example:
...
causes two of the cascoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the cascoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the cascoin source code).
