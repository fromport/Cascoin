...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current Cascoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account cascoin address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Cascoin address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The cascoin address to be associated with an account.\n"
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The cascoin address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a cascoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The cascoin address to send to.\n"
            ...
            "                             The recipient will receive less cascoin than you enter in the amount field.\n"
...
UniValue createbees(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "createmice mice_count ( community_contrib, \"honey_address\", \"change_address\" )\n"
            "\nCreate one or more mice in a single transaction, sign it, and broadcast it to the network.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. mice_count              (numeric, required) The number of mice to create.\n"
...
// Cascoin: Rialto: Check if a nick is registered
...
/*
// Cascoin: Rialto: Retrieve a Rialto pubkey for a given nick from the White Pages DB
...
// Cascoin: Rialto: List our own Rialto nicks from the local white pages
...
// Cascoin: Rialto: Rebuild the Rialto white pages databases by replaying since first Rialto block.
...
// Cascoin: Rialto: Check if given nick is blocked
...
// Cascoin: Rialto: Get all incoming messages (longpoll)
...
// Cascoin: Rialto: Encrypt a message
...
/*
// Cascoin: Rialto: Decrypt a message
...
// Cascoin: Generic NFT System: Create a generic NFT
UniValue nftcreate(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 4)
        throw std::runtime_error(
            "nftcreate \"name\" \"description\" \"metadata\" amount\n"
            "\nCreate a generic NFT.\n"
            "\nArguments:\n"
            "1. \"name\"        (string, required) The name of the NFT\n"
            "2. \"description\" (string, required) Description of the NFT\n"
            "3. \"metadata\"    (string, required) JSON metadata for the NFT\n"
            "4. amount         (numeric, required) Amount of the NFT\n"
            "\nResult:\n"
            "{\n"
            "  \"txid\": \"transaction_id\",\n"
            "  \"nft_id\": \"nft_identifier\"\n"
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("nftcreate", "\"My NFT\" \"A cool NFT\" \"{\\\"rarity\\\":\\\"rare\\\"}\" 1000")
            + HelpExampleRpc("nftcreate", "\"My NFT\", \"A cool NFT\", \"{\\\"rarity\\\":\\\"rare\\\"}\", 1000")
        );
    ...
    std::string metadata = request.params[2].get_str();
    int64_t amount = request.params[3].get_int64();

    if (name.empty()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "NFT name cannot be empty");
    }
    ...
    std::string ownerAddress = EncodeDestination(dest);

    // Create generic NFT
    GenericNFT nft(name, description, metadata, amount, ownerAddress);
    std::vector<GenericNFT> nfts = {nft};
    ...
    // Create OP_RETURN output for NFT data
    ...
    // Create dust output to owner address
    CScript ownerScript = GetScriptForDestination(dest);
    vecSend.push_back({ownerScript, 546, false}); // Dust amount

    CWalletTx wtxNew;
    CCoinControl coin_control;
    if (!pwallet->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError, coin_control)) {
        if (amount + nFeeRequired > pwallet->GetBalance())
    ...
    result.pushKV("nft_id", wtxNew.GetHash().GetHex());
    result.pushKV("name", name);
    result.pushKV("amount", amount);
    result.pushKV("owner_address", ownerAddress);

    return result;
}

// Cascoin: Generic NFT System: Transfer a generic NFT
UniValue nfttransfer(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "nfttransfer \"nft_txid\" \"recipient_address\" amount\n"
            "\nTransfer a generic NFT to another address.\n"
            "\nArguments:\n"
            "1. \"nft_txid\"        (string, required) The transaction ID of the NFT to transfer\n"
            "2. \"recipient_address\" (string, required) The address to transfer the NFT to\n"
            "3. amount              (numeric, required) Amount of the NFT to transfer\n"
            "\nResult:\n"
            "{\n"
            "  \"txid\": \"transaction_id\"\n"
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("nfttransfer", "\"abc123...\" \"CQj1...\" 1000")
            + HelpExampleRpc("nfttransfer", "\"abc123...\", \"CQj1...\", 1000")
        );

    LOCK2(cs_main, pwallet->cs_wallet);

    std::string nftTxid = request.params[0].get_str();
    std::string recipientAddress = request.params[1].get_str();
    ...
    // Find the NFT transaction
    uint256 txHash;
    txHash.SetHex(nftTxid);
    
    auto it = pwallet->mapWallet.find(txHash);
    if (it == pwallet->mapWallet.end()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "NFT transaction not found in wallet");
    }

    const CWalletTx& nftTx = it->second;
    
    // Check if NFT output is spent
    if (pwallet->IsSpent(txHash, 0)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "NFT has already been transferred");
    }

    // Verify ownership (check if we own the dust output)
    if (!pwallet->IsMine(nftTx.tx->vout[1])) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "You don't own this NFT");
    }
    ...
    // Create OP_RETURN output for transfer data
    GenericNFTTransfer transfer(nftTxid, recipientAddress, amount);
    std::vector<GenericNFTTransfer> transfers = {transfer};
    std::vector<unsigned char> transferData = CreateGenericNFTTransferScript(transfers);
    CScript transferScript = CScript() << OP_RETURN << transferData;
    vecSend.push_back({transferScript, 0, false});

    // Create dust output to recipient
    CScript recipientScript = GetScriptForDestination(dest);
    vecSend.push_back({recipientScript, 546, false}); // Dust amount

    // Use coin control to spend the specific NFT UTXO
    CCoinControl coin_control;
    coin_control.Select(COutPoint(txHash, 1)); // Use the dust output as input
    ...
    UniValue result(UniValue::VOBJ);
    result.pushKV("txid", wtxNew.GetHash().GetHex());
    result.pushKV("nft_txid", nftTxid);
    result.pushKV("recipient", recipientAddress);
    result.pushKV("amount", amount);

    return result;
}

// Cascoin: Generic NFT System: List all generic NFTs
UniValue nftlist(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "nftlist\n"
            "\nList all generic NFTs owned by this wallet.\n"
            "\nResult:\n"
            "[\n"
            "  {\n"
            "    \"txid\": \"transaction_id\",\n"
    ...
    for (const auto& item : pwallet->mapWallet) {
        ...
        // Check if this is a generic NFT transaction
        std::string error;
        if (IsValidGenericNFTTransaction(*wtx.tx, error)) {
            std::vector<GenericNFT> nfts;
            if (ParseGenericNFTTransaction(*wtx.tx, nfts, error)) {
                for (const auto& nft : nfts) {
                    // Check if we own the NFT (dust output)
                    if (wtx.tx->vout.size() > 1 && pwallet->IsMine(wtx.tx->vout[1])) {
                        // Check if NFT is spent
                        if (!pwallet->IsSpent(wtx.GetHash(), 1)) {
                            UniValue nftObj(UniValue::VOBJ);
                            nftObj.pushKV("txid", wtx.GetHash().GetHex());
                            nftObj.pushKV("name", nft.name);
                            nftObj.pushKV("description", nft.description);
                            nftObj.pushKV("metadata", nft.metadata);
                            nftObj.pushKV("amount", nft.amount);
                            nftObj.pushKV("owner_address", nft.owner_address);
                            nftObj.pushKV("confirmed", wtx.GetDepthInMainChain() > 0);
                            result.push_back(nftObj);
                        }
                    }
                }
            }
...
// Cascoin: Generic NFT System: Create NFT with large metadata
UniValue nftcreate_large(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 5)
        throw std::runtime_error(
            "nftcreate_large \"name\" \"description\" \"metadata\" \"image_data\" amount\n"
            "\nCreate a generic NFT with large metadata stored directly on the blockchain.\n"
            "\nArguments:\n"
            "1. \"name\"        (string, required) The name of the NFT\n"
            "2. \"description\" (string, required) Description of the NFT\n"
            "3. \"metadata\"    (string, required) JSON metadata for the NFT\n"
            "4. \"image_data\"  (string, required) Base64 encoded image data (max 3KB)\n"
            "5. amount         (numeric, required) Amount of the NFT\n"
            "\nResult:\n"
            "{\n"
            "  \"txid\": \"transaction_id\",\n"
            "  \"nft_id\": \"nft_identifier\",\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("nftcreate_large", "\"My NFT\" \"A cool NFT\" \"{\\\"rarity\\\":\\\"rare\\\"}\" \"iVBORw0KGgoAAAANSUhEUgAA...\" 1000")
            + HelpExampleRpc("nftcreate_large", "\"My NFT\", \"A cool NFT\", \"{\\\"rarity\\\":\\\"rare\\\"}\", \"iVBORw0KGgoAAAANSUhEUgAA...\", 1000")
        );
    ...
    std::string description = request.params[1].get_str();
    std::string metadata = request.params[2].get_str();
    std::string imageData = request.params[3].get_str();
    int64_t amount = request.params[4].get_int64();
    ...
    // Create generic NFT with large metadata
    GenericNFT nft(name, description, metadata + "|IMAGE:" + imageData, amount, ownerAddress);
    std::vector<GenericNFT> nfts = {nft};
    ...
    result.pushKV("amount", amount);
    result.pushKV("owner_address", ownerAddress);
    result.pushKV("data_size", totalDataSize);
...
UniValue getnetworkhiveinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  immature_mice_count,             (numeric) The number of Resting mice\n"
            "  immature_bct_count,             (numeric) The number of Resting Mouse Creation Transactions\n"
            "  mature_mice_count,               (numeric) The number of Adventure mice\n"
    ...
    jsonResults.push_back(Pair("immature_mice_count", globalImmatureBees));
    jsonResults.push_back(Pair("immature_bct_count", globalImmatureBCTs));
    jsonResults.push_back(Pair("mature_mice_count", globalMatureBees));
...
// Cascoin: Hive: Return BCT tx id for a honey transaction in this wallet
...
UniValue getbctinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "    mice_count,            (numeric) The number of mice created\n"
    ...
    if (bct.txid != "") {
        ...
        jsonResults.push_back(Pair("mice_count", bct.beeCount));
...
UniValue gethiveinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "{\n"
            "    summary: {\n"
            "        mice_count,                (numeric) Total mouse count in labyrinth\n"
    ...
    for (std::vector<CBeeCreationTransactionInfo>::const_iterator it = bcts.begin(); it != bcts.end(); it++) {
        ...
        if (bct.txid!="") {
            ...
            entry.push_back(Pair("mice_count", bct.beeCount));
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The cascoin address\n"
            "      amount,                 (numeric) The amount in " + CURRENCY_UNIT + "\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The cascoin address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The cascoin address for transactions.\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a cascoin address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The cascoin address to send funds to.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The cascoin address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less cascoin than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Cascoin address: ") + name_);
        }
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" \"address_type\" )\n"
            ...
            "Each key is a Cascoin address or hex-encoded public key.\n"
            ...
            "2. \"keys\"                         (string, required) A json array of cascoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"                  (string) cascoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            ...
            "}\n"
            "\nResult (DEPRECATED. To see this result in v0.16 instead, please start cascoind with -deprecatedrpc=addmultisigaddress).\n"
            "        clients should transition to the new output api before upgrading to v0.17.\n"
            "\"address\"                         (string) A cascoin address associated with the keys.\n"
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart cascoind with -deprecatedrpc=addwitnessaddress.\n"
...
void ListTransactions(CWallet* const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        for (const COutputEntry& s : listSent)
        {
            ...
            // Cascoin: Hive: Sent transactions are never honey
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        for (const COutputEntry& r : listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                // Cascoin: Hive: Indicate whether this is honey (hive block coinbase tx)
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    // Cascoin: Hive: Include ishoney in documentation
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The cascoin address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The cascoin address involved in the transaction\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending cascoin\n"
            "\nArguments:\n"
...
// Cascoin: Rialto: Get the current wallet lock status
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending cascoin\n"
    ...
    StartShutdown();
    return "wallet encrypted; Cascoin server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending cascoin.\n"
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of cascoin addresses to filter\n"
            "    [\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the cascoin address\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Cascoin address: ") + input.get_str());
            }
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The cascoin address to receive the change\n"
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid cascoin address");
            }
...
{ //  category              name                        actor (function)           argNames
    ...
    { "wallet",             "createmice",               &createbees,               {"mice_count","community_contrib","honey_address"} },     // Cascoin: Hive: Create bee(s)
    ...
    { "wallet",             "getbctinfo",               &getbctinfo,               {"bct_txid","min_honey_confirms"} },                     // Cascoin: Hive: Return hive info for a single BCT
...
//    { "wallet",             "rialtogetpubkey",          &rialtogetpubkey,          {"nickname"} },                                          // Cascoin: Rialto: Get a Rialto pubkey
    ...
    { "wallet",             "rialtogetmynicks",         &rialtogetmynicks,         {} },                                                    // Cascoin: Rialto: Get my Rialto addresses for outgoing messages
    { "wallet",             "rialtorebuildwhitepages",  &rialtorebuildwhitepages,  {} },                                                    // Cascoin: Rialto: Rebuild the Rialto white pages DBs
    { "wallet",             "rialtoencrypt",            &rialtoencrypt,            {"sender_nick","destination_nick","message"} },          // Cascoin: Rialto: Encrypt a message
//    { "wallet",             "rialtodecrypt",            &rialtodecrypt,            {"hexstring"} },                                         // Cascoin: Rialto: Try to decrypt a message with all available Rialto private keys
    { "wallet",             "rialtoblocknick",          &rialtoblocknick,          {"nickname"} },                                          // Cascoin: Rialto: Block given nick
    { "wallet",             "rialtounblocknick",        &rialtounblocknick,        {"nickname"} },                                          // Cascoin: Rialto: Unblock given nick
    ...
    { "wallet",             "nftcreate",                &nftcreate,                {"name","description","metadata","amount"} },            // Cascoin: Generic NFT System: Create generic NFT
    { "wallet",             "nfttransfer",              &nfttransfer,              {"nft_txid","recipient_address","amount"} },             // Cascoin: Generic NFT System: Transfer generic NFT
    { "wallet",             "nftlist",                  &nftlist,                  {} },                                                    // Cascoin: Generic NFT System: List generic NFTs
    { "wallet",             "nftcreate_large",          &nftcreate_large,          {"name","description","metadata","image_data","amount"} }, // Cascoin: Generic NFT System: Create NFT with large metadata
    ...
    { "wallet",             "walletcandecryptrialto",   &walletcandecryptrialto,   {} },                            // Cascoin: Rialto: Get wallet lock status
