    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!SequenceLocks(tx, nLockTimeFlags, &prevheights, *pindex)) {
                return state.DoS(100, error("%s: contains a non-BIP68-final transaction"),
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        if (miSelf != mapBlockIndex.end()) {
            ...
            if (ppindex)
                *ppindex = pindex;
            if (pindex->nStatus & BLOCK_FAILED_MASK)
                return state.Invalid(error("AcceptBlock(): block %s is marked invalid", hash.ToString()), 0, "duplicate");
            return true;
        }

        if (!CheckBlockHeader(block, state, chainparams.GetConsensus()))
            return error("%s: CheckBlockHeader failed: %s", __func__, FormatStateMessage(state));
        ...
        // Special case for first 100 blocks - allow mining with minimum difficulty
        if (pindexPrev && pindexPrev->nHeight < 100) {
            if (!CheckBlock(block, state, chainparams.GetConsensus(), false, true))
        ...
        // Verify timestamp
        if (pindexPrev->GetMedianTimePast() >= block.GetBlockTime())
        ...
        if (IsWitnessEnabled(pindexPrev, chainparams.GetConsensus())) {
            ...
            if (commitpos != -1) {
                ...
                // witness tree.
                if (block.vtx[0]->vin[0].scriptWitness.stack.size() != 1 || block.vtx[0]->vin[0].scriptWitness.stack[0].size() != 32) {
                    return state.DoS(100, error("AcceptBlock(): invalid witness nonce size"), REJECT_INVALID, "bad-witness-nonce-size");
                }
                CHash256().Write(hashWitness.begin(), 32).Write(&block.vtx[0]->vin[0].scriptWitness.stack[0][0], 32).Finalize(hashWitness.begin());
                if (memcmp(hashWitness.begin(), &block.vtx[0]->vout[commitpos].scriptPubKey[6], 32)) {
                    return state.DoS(100, error("AcceptBlock(): witness merkle commitment mismatch"), REJECT_INVALID, "bad-witness-merkle-match");
                }
                fHaveWitness = true;
            }
        }
        ...
        if (!fHaveWitness) {
            for (const auto& tx : block.vtx) {
                if (tx->HasWitness()) {
                    return state.DoS(100, error("AcceptBlock(): unexpected witness data found"), REJECT_INVALID, "unexpected-witness");
                }
            }
        }
        ...
        // failed).
        if (GetBlockWeight(block) > MAX_BLOCK_WEIGHT) {
            return state.DoS(100, error("AcceptBlock(): weight limit failed"), REJECT_INVALID, "bad-blk-weight");
        }
    }
