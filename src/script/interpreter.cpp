...
bool static IsDefinedHashtypeSignature(const valtype &vchSig) {
    ...
    unsigned char nHashType = vchSig[vchSig.size() - 1] & (~(SIGHASH_ANYONECANPAY | SIGHASH_FORKID));   // Cascoin: Use SIGHASH_FORKID
...
// Cascoin: Drop the signature in scripts when SIGHASH_FORKID is not used
...
// Cascoin: Check fork ID before calling FindAndDelete
...
bool CheckSignatureEncoding(const std::vector<unsigned char> &vchSig, unsigned int flags, ScriptError* serror) {
    ...
    if ((flags & (SCRIPT_VERIFY_DERSIG | SCRIPT_VERIFY_LOW_S | SCRIPT_VERIFY_STRICTENC)) != 0 && !IsValidSignatureEncoding(vchSig)) {
        ...
        // serror is set
        return false;
    } else if ((flags & SCRIPT_VERIFY_STRICTENC) != 0) {            // Cascoin: Enforce fork ID checking
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGVERIFY:
                {
                    ...
                    if (sigversion == SIGVERSION_BASE) {
                        CleanupScriptCode(scriptCode, vchSig, flags);   // Cascoin: Check fork ID before calling FindAndDelete
                    }
                ...
                // Cascoin: Mice NFT System - REMOVED opcodes for true soft fork compatibility
                // NFT validation now uses magic bytes in OP_RETURN data instead of opcodes
