...
ThresholdState AbstractThresholdConditionChecker::GetStateFor(const CBlockIndex* pindexPrev, const Consensus::Params& params, int nPeriod, int nThreshold, int64_t nTimeStart, int64_t nTimeTimeout, int nWindowSize, int nMinLockedBlocks, int nLockPeriodBlocks, Consensus::DeploymentPos dep_pos, VersionBitsCache& cache)
{
    // Check if this deployment is always active.
    if (nTimeStart == 0 && nTimeTimeout == Consensus::BIP9Deployment::NO_TIMEOUT && nPeriod == 0 && nThreshold == 0 && nWindowSize ==0) { // Cascoin: Added nWindowSize == 0 for Litecoin Cash "always active"
        return THRESHOLD_ACTIVE;
    }

    // Check if this deployment is hidden defined by nTimeStart == 0 && nTimeTimeout == 0
    if (nTimeStart == 0 && nTimeTimeout == 0) {
        return THRESHOLD_HIDDEN;
    }

    // Handle pindexPrev == nullptr explicitly at the beginning.
    if (pindexPrev == nullptr) {
        if (GetAdjustedTime() < nTimeStart) {
    ...
    if (nTimePrev < nTimeStart) {
        return THRESHOLD_DEFINED;
    }

    // A block's state is always the same as that of the first of its period, so it is computed based on a pindexPrev whose height equals a multiple of nPeriod - 1.
    const CBlockIndex* pindexPeriod = pindexPrev->GetAncestor(pindexPrev->nHeight - ((pindexPrev->nHeight + 1) % nPeriod));

    // Walk backwards in steps of nPeriod to find a pindexPeriod whose information is known
    std::vector<const CBlockIndex*> vToCompute;
    const CBlockIndex* pindexWalk = pindexPeriod;
    while (cache.find(pindexWalk) == cache.end()) {
        if (pindexWalk->GetMedianTimePast() < nTimeStart) {
            // Optimization: don't recompute down further, as we know every earlier block will be before the start time
            cache[pindexWalk] = THRESHOLD_DEFINED;
            break;
        }
        if (!pindexWalk->pprev) {
            // The genesis block is by definition defined.
        ...
        pindexWalk = pindexWalk->GetAncestor(pindexWalk->nHeight - nPeriod);
    }

    // At this point, cache[pindexWalk] is known
    assert(cache.find(pindexWalk) != cache.end());
    ThresholdState state = cache[pindexWalk];
    ...
    while (!vToCompute.empty()) {
        ...
        switch (state) {
            case THRESHOLD_DEFINED: {
                if (pindexWalk->GetMedianTimePast() >= nTimeTimeout) {
                    stateNext = THRESHOLD_FAILED;
                } else if (pindexWalk->GetMedianTimePast() >= nTimeStart) {
                    stateNext = THRESHOLD_STARTED;
                }
                break;
            }
        ...
        cache[pindexWalk] = state = stateNext;
    }
