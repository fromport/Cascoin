...
const QString BITCOIN_IPC_PREFIX("cascoin:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/cascoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/cascoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/cascoin-paymentrequest";
...
#else
        certList = QList<QSslCertificate>();
#endif
    }
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start cascoin: click-to-pay handler"));
        }
        else {
...
#else
    qDebug() << "PaymentServer::initNetManager: Network proxy support not available in Qt build.";
#endif
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Cascoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::reportSslErrors(QNetworkReply* reply, const QList<QSslError> &errs)
{
    ...
    for (const QSslError& err : errs) {
        ...
        errString += QString("SSL error: %1\n").arg(QString::fromUtf8(QSslError(err).errorString().toUtf8()));
#else
        errString += QString("SSL error: %1\n").arg(err.errorString());
#endif
    }
